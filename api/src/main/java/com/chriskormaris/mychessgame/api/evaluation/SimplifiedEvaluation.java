...
public class SimplifiedEvaluation implements Evaluation {
	...
	private int getPieceCentipawnValue(ChessSquare square) {
		if (square.isPawn()) {
	...
	public double evaluate(ChessBoard chessBoard) {
		...
		for (int i = 0; i < chessBoard.getNumOfRows(); i++) {
			for (int j = 0; j < chessBoard.getNumOfColumns(); j++) {
				...
				if (square.isWhite()) {
					score += PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square);
					score += getPieceSquareValue(i, j, square, gamePhase);
				} else if (square.isBlack()) {
					score -= PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square);

					int row = chessBoard.getNumOfRows() - 1 - i;
					score -= getPieceSquareValue(row, j, square, gamePhase);
				}
			}
		}
