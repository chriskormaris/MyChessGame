...
public class PeSTOEvaluation implements Evaluation {
	...
	private static final int[][] MIDDLEGAME_PAWN_SQUARE_TABLE = new int[][]{
			{0,    0,  0,    0,   0,   0,  0,   0},
	...
	private static final int[][] MIDDLEGAME_KNIGHT_SQUARE_TABLE = new int[][]{
	...
	private static final int[][] MIDDLEGAME_BISHOP_SQUARE_TABLE = new int[][]{
	...
	private static final int[][] MIDDLEGAME_ROOK_SQUARE_TABLE = new int[][]{
	...
	private static final int[][] MIDDLEGAME_QUEEN_SQUARE_TABLE = new int[][]{
	...
	private static final int MIDDLEGAME_PAWN_CENTIPAWN_VALUE = 82;
	private static final int MIDDLEGAME_KNIGHT_CENTIPAWN_VALUE = 337;
	private static final int MIDDLEGAME_BISHOP_CENTIPAWN_VALUE = 365;
	private static final int MIDDLEGAME_ROOK_CENTIPAWN_VALUE = 477;
	private static final int MIDDLEGAME_QUEEN_CENTIPAWN_VALUE = 1025;
	private static final int MIDDLEGAME_KING_CENTIPAWN_VALUE = 0;

	private static final int ENDGAME_PAWN_CENTIPAWN_VALUE = 94;
	private static final int ENDGAME_KNIGHT_CENTIPAWN_VALUE = 281;
	private static final int ENDGAME_BISHOP_CENTIPAWN_VALUE = 297;
	private static final int ENDGAME_ROOK_CENTIPAWN_VALUE = 512;
	private static final int ENDGAME_QUEEN_CENTIPAWN_VALUE = 936;
	private static final int ENDGAME_KING_CENTIPAWN_VALUE = 0;


	private int getPieceCentipawnValue(ChessSquare square, GamePhase gamePhase) {
	...
	private int getMiddlegamePieceSquareValue(int row, int column, ChessSquare square) {
		if (square.isPawn()) {
	...
	private int getEndgamePieceSquareValue(int row, int column, ChessSquare square) {
		if (square.isPawn()) {
	...
	private int getPieceSquareValue(int row, int column, ChessSquare square, GamePhase gamePhase) {
		if (gamePhase == GamePhase.OPENING_MIDDLEGAME) {
			return getMiddlegamePieceSquareValue(row, column, square);
		} else {
			return getEndgamePieceSquareValue(row, column, square);
		}
	}
	...
	public double evaluate(ChessBoard chessBoard) {
		...
		for (int i = 0; i < chessBoard.getNumOfRows(); i++) {
			for (int j = 0; j < chessBoard.getNumOfColumns(); j++) {
				...
				gamePhase += getPieceGamePhaseValue(square);

				if (square.isWhite()) {
					middlegameScore += PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square, GamePhase.OPENING_MIDDLEGAME);
					endgameScore += PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square, GamePhase.ENDGAME);

					middlegameScore += getPieceSquareValue(i, j, square, GamePhase.OPENING_MIDDLEGAME);
					endgameScore += getPieceSquareValue(i, j, square, GamePhase.ENDGAME);
				} else if (square.isBlack()) {
					middlegameScore -= PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square, GamePhase.OPENING_MIDDLEGAME);
					endgameScore -= PIECE_VALUE_MULTIPLIER * getPieceCentipawnValue(square, GamePhase.ENDGAME);

					int row = chessBoard.getNumOfRows() - 1 - i;
					middlegameScore -= getPieceSquareValue(row, j, square, GamePhase.OPENING_MIDDLEGAME);
					endgameScore -= getPieceSquareValue(row, j, square, GamePhase.ENDGAME);
				}
			}
		}
